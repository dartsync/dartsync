...
int watchDirectory(peer_file_table* ptable,char* directory){
    ...
    while (1) {
        ...
        while(1){
                        ...
                        if(event->mask & IN_CREATE){
                            ...
                            if(isdir){
                                ...
                                if(!ignore_tmp(filename)){
                                    ...
                                    int t = fileModified(ptable,tmpname);
                                    if(t == 1){
                                        if(updated == 0)
                                            updated = 1;
                                        flag = 1;
            ...
            gettimeofday(&intervalTime, NULL);
            unsigned long int cur_time = intervalTime.tv_sec * 1000000 + intervalTime.tv_usec;
            if(cur_time - time > 3 * 1000000){
                break;
            }
        }
        printf("updated = %d\n", updated);
        if(updated){
            ...
            printf("download table is not empty\n");
        }
    }
